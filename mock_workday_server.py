...
ORG_MANAGER = {
    "Finance":       ("Sandeep Rao",     "+1-972-555-0101", "+1-972-555-0102"),
    "HR Operations": ("Laura Bennett",   "+1-469-555-0111", "+1-469-555-0112"),
    "IT Services":   ("Daniel Okoro",    "+1-214-555-0121", "+1-214-555-0122"),
    "Audit":         ("Priscilla Vance", "+1-972-555-0131", "+1-972-555-0132"),
    "Legal":         ("Marco Bianchi",   "+1-469-555-0141", "+1-469-555-0142"),
}
...
for i in range(23):
    ...
    ec_email = ec_name.lower().replace(" ", ".") + "@personal.demo"
    # Data-quality gaps on purpose (integration error-handling practice):
    #   every 7th worker (i % 7 == 3)  -> no emergency contact phone
    #   every 11th worker (i % 11 == 5) -> no emergency contact email
    if i % 7 == 3:
        ec_phone = ""
    if i % 11 == 5:
        ec_email = ""
    ...
    mgr_name, mgr_p1, mgr_p2 = ORG_MANAGER[org]
    ...
    WORKERS.append({
        ...
        # Demographic outbound fields (App Catalog employeeDemographicOutbound sample)
        "Preferred_First_Name": first if i % 6 else first[:3],
        "Legal_First_Name": first,
        "Legal_Last_Name": last,
        "Original_Hire_Date": f"20{16 + (i % 7)}-0{1 + (i % 9)}-15" if i % 5 == 0 else f"20{18 + (i % 7)}-0{1 + (i % 9)}-15",
        "Termination_Date": "" if i % 9 != 8 else "2026-06-30",
        "Pay_Rate_Type": "Hourly" if i % 4 == 0 else "Salary",
        "Time_Type": "Part_time" if i % 8 == 7 else "Full_time",
        "Worker_Type": "Contingent_Worker" if i % 10 == 9 else "Employee",
        "Manager_Employee_ID": "" if eid == "21001" else "21001",
        "Location": ["Irving", "Dallas", "Plano", "Remote"][i % 4],
        "Position_ID": f"P-{100000 + i}",
        "Cost_Center": f"CC_{org.upper().replace(' ', '_')}",
        "Job_Code": f"J{1000 + (i % 6)}",
        "Business_Title": ["Analyst", "Sr Analyst", "Consultant", "Sr Consultant", "Manager", "Director"][i % 6],
...
BASE_ALIASES = {  # report column label -> worker key
    ...
    # Emergency Contact business object (report label -> worker key)
    ...
    "Phone_Primary": "Emergency_Contact_Phone",
    "Primary Email": "Emergency_Contact_Email",
}
...
def eval_calc_field(defn, ctx):
    ...
    if fn == "Lookup Related Value":               # LRV
        ...
        if isinstance(obj, dict):
            return str(obj.get(defn.get("return_field", ""), ""))
        if isinstance(obj, list) and obj:            # tolerate a 1-item list
            return str(obj[0].get(defn.get("return_field", ""), ""))
        return ""

    if fn == "Evaluate Expression":                # EE (ordered If/Then/Else + default)
        def _resolve(v):
            # a return/default may be a literal, or the name of another calc
            # field / worker field (e.g. a Text Constant M / F / NA)
            return "" if v is None else str(ctx.get(v, v))
        for c in defn.get("conditions", []):
            if "field" in c:                       # comparison row
                hit = _flt_match(ctx.get(c.get("field"), ""),
                                 c.get("op", "equal to"), c.get("value", ""))
            else:                                  # references a True/False calc field
                hit = str(ctx.get(c.get("condition"), "")).lower() == "true"
            if hit:
                return _resolve(c.get("return", ""))
        return _resolve(defn.get("default", ""))

    if fn == "True/False Condition":               # TF
        left = ctx.get(defn.get("condition_field"), "")
        op = defn.get("operator", "equals")
        right = defn.get("value", "")
        if op == "equals":
    ...
    if fn == "Extract Single Instance":            # ESI
        ...
        return str(chosen.get(rf, "")) if rf else str(chosen)
...
def worker_row(worker, columns):
    ...
    for defn in CALC_FIELDS:
        try:
            ...
            ctx[defn.get("field_name", "")] = ""   # one bad calc field must not 500 every report
    row = {}
    for col in columns:
...
def _flt_match(val, op, target):
    """Compare a value against a target using a Workday filter operator.
    Mirrors the Report Writer UI semantics so RaaS honors the same filters."""
    ...
    if op == "less than or equal to":
        return nv is not None and nt is not None and nv <= nt
    if op == "not equal to":
        return (nv != nt) if (nv is not None and nt is not None) \
            else (s.lower() != t.lower())
    return (nv == nt) if (nv is not None and nt is not None) \
        else (s.lower() == t.lower())


def passes_conditions(ctx, conds):
    """Filter on Instances for RaaS: alias-aware field lookup, per-row And/Or
    combined left-to-right (same as the Report Writer grid)."""
    if not conds:
        return True
    result = None
    for c in conds:
        key = BASE_ALIASES.get(c["field"], c["field"])
        ok = _flt_match(ctx.get(key, ""), c.get("op", "equal to"),
                        c.get("value", ""))
        if result is None:
            result = ok
        elif (c.get("andor", "And") or "And").lower() == "or":
            result = result or ok
        else:
...
def resolve_instances(worker, business_object, field):
    ...
    if bo in RELATED_OBJECTS:
        ...
        obj = worker.get(key)
        if card == "multi":
            return [str(inst.get(field, "")) for inst in (obj or [])]
...
def raas_report(report_name):
    ...
    # Build rows (apply calc fields), then apply report filters.
    # Filter on Instances (the Report Writer grid) applies to RaaS output
    # exactly like it does in the UI - real RaaS respects report filters.
    workers = WORKERS
    fconds = rpt.get("filter_conditions")
    if fconds:
        workers = [w for w in workers
                   if passes_conditions(worker_context(w), fconds)]
    flt = rpt.get("filter")
    if flt:
        workers = [w for w in workers
                   if worker_row(w, rpt["columns"]).get(flt["field"]) == flt["equals"]]
    for f_field, f_val in prompt_filters:
        # Workday multi-instance prompts arrive as Workers!Employee_ID=21001!21002
        wanted = {v for v in str(f_val).split("!") if v != ""}
        f_key = f_field.split("!")[-1] if "!" in f_field else f_field
        workers = [w for w in workers
                   if str(worker_row(w, rpt["columns"] + [f_key]).get(f_key, "")) in wanted]

    # Column definitions carry the business object + heading. RaaS uses the
    # report heading as the field name and clubs multi-instance related
    # objects (Dependents / Payments / Benefits) exactly like the grid.
    cdefs = rpt.get("column_defs") or [{"field": c, "business_object": "Worker"}
                                       for c in rpt["columns"]]

    def _label(cd):
        return (cd.get("heading") or "").strip() or cd["field"]

    def _vals(w, cd):
        return [x for x in resolve_instances(
            w, cd.get("business_object", "Worker"), cd["field"]) if x != ""]

    entries = [[(_label(cd), _vals(w, cd)) for cd in cdefs] for w in workers]
    labels = [_label(cd) for cd in cdefs]
    fmt = request.args.get("format", "xml").lower()

    if fmt == "json":
        data = {"Report_Entry": [
            {lbl: (vs[0] if len(vs) == 1 else vs) if vs else "" for lbl, vs in row}
            for row in entries]}
    ...
    if fmt == "csv":
        ...
        wtr.writerow(labels)
        for row in entries:
            wtr.writerow(["; ".join(vs) for _l, vs in row])
    ...
    def _tag(lbl):
        t = re.sub(r"[^A-Za-z0-9_]", "_", str(lbl)).strip("_")
        return t if (t and re.match(r"[A-Za-z_]", t[0])) else "_" + t

    simple = (fmt == "simplexml")
    pre = "" if simple else "wd:"
    ns = "" if simple else f' xmlns:wd="urn:com.workday.report/{report_name}"'
    out = ['<?xml version="1.0" encoding="UTF-8"?>', f"<{pre}Report_Data{ns}>"]
    for row in entries:
        out.append(f"  <{pre}Report_Entry>")
        for lbl, vs in row:
            tag = _tag(lbl)
            if not vs:
                out.append(f"    <{pre}{tag}/>")
            for x in vs:
                out.append(f"    <{pre}{tag}>{_xesc(x)}</{pre}{tag}>")
        out.append(f"  </{pre}Report_Entry>")
    out.append(f"</{pre}Report_Data>")
...
if __name__ == "__main__":
    print("Mock Workday tenant -> http://localhost:8443")
