...
.rpt-x{border:1px solid #c7cdd4;background:#fff;border-radius:16px;padding:6px 14px;
  font-size:13px;color:#2E6DA4;font-weight:600;text-decoration:none;white-space:nowrap}
.rpt-x:hover{background:#F0F6FC}
.rpt-actions .wd-count{margin:0;color:#555;font-weight:600}
.raas-ws{background:#F0F6FC;border:1px solid #cfe0f0;border-radius:8px;
  padding:10px 14px;font-size:12.5px;color:#33475b;margin:6px 0 2px}
.raas-ws code{background:#fff;border:1px solid #d6dbe0;border-radius:4px;
  padding:2px 6px;color:#2E6DA4}
.raas-ws .raas-tag{font-weight:700;color:#2E6DA4;margin-right:8px}
.raas-fmts{margin-top:6px}
.raas-fmts a{color:#0875e1;font-weight:600;text-decoration:none}
.raas-fmts a:hover{text-decoration:underline}
.raas-ws.off{background:#fff6f6;border-color:#f2c7c7;color:#a23}
</style>"""
...
# View / run reports
# ---------------------------------------------------------------------------
def _resolve_report(name, args):
    """Shared report resolution used by BOTH the grid view and the RaaS web
    service so their data is always identical. Returns a dict with either an
    'error' (code, msg), a 'missing' prompt list, or the resolved rows."""
    rpt = mws.REPORTS.get(name)
    if not rpt:
        return {"error": (404, "Report not found: %s" % name)}
    prompts = rpt.get("prompts", [])

    def _pv(pr):
        return (args.get("p_" + pr["name"], "") or args.get(pr["name"], "")).strip()

    missing = [pr for pr in prompts if pr.get("required") and not _pv(pr)]
    if prompts and missing:
        return {"error": None, "missing": missing, "rpt": rpt, "prompts": prompts}

    cdefs = rpt.get("column_defs")
    if not cdefs:
        cdefs = [{"field": c, "business_object": "Worker"} for c in rpt["columns"]]
    ...
    if sub and sub.get("business_object"):
        ...
        _c = ", ".join(("%s %s %s" % (c["field"], c["op"], c["value"])).strip()
                       for c in sub.get("conditions", []))
        sub_note = ' &middot; subfilter: <b>%s</b> where %s' % (sub["business_object"], _c)
    elif sub and sub.get("field"):
        pairs = [(w, r) for (w, r) in pairs
                 if str(r.get(sub["field"])) == str(sub["equals"])]
        sub_note = ' &middot; subfilter: <b>%s=%s</b>' % (sub["field"], sub["equals"])
    ...
    headings = {d["field"]: d.get("heading") for d in cdefs if d.get("heading")}
    return {"error": None, "missing": [], "rpt": rpt, "prompts": prompts,
            "cdefs": cdefs, "pairs": pairs, "applied": applied,
            "sub_note": sub_note, "headings": headings}
...
{rows}</table></div>"""
    ...
    res = _resolve_report(name, request.args)
    if res.get("error"):
        _code, _msg = res["error"]
        return html_resp(layout("View Custom Report", "",
                                '<div class="err-banner">%s</div>' % _msg))
    if res.get("missing"):
        prompts = res["prompts"]
        fields = "".join(
            '<label>%s <span class="req">*</span></label>'
            '<input type="text" name="p_%s" value="%s" placeholder="enter value to run">'
            % (pr["name"], pr["name"], request.args.get("p_" + pr["name"], ""))
            for pr in prompts)
...
<form method="get"><div class="card">
  ...
  <div class="btnrow"><button class="btn btn-ok" type="submit">OK</button>
...
</div></form>""" % (warn, name, fields)
        return html_resp(layout(name, "Enter prompt values (launch parameters)", body))

    rpt = res["rpt"]
    cdefs = res["cdefs"]
    pairs = res["pairs"]
    applied = res["applied"]
    sub_note = res["sub_note"]
    ...
    crit = (' &middot; prompts applied: <b>%s</b>' % ", ".join(applied)) if applied else ""
    raas_base = "/ccx/service/customreport2/%s/ISU_Demo/%s" % (TENANT, name)
    if rpt.get("enable_as_web_service"):
        fl = " &middot; ".join(
            '<a href="%s?format=%s" target="_blank">%s</a>' % (raas_base, ff, lbl)
            for ff, lbl in [("json", "JSON"), ("xml", "XML"), ("csv", "CSV"),
                            ("simplexml", "Simple XML")])
        ws_block = ('<div class="raas-ws"><span class="raas-tag">Web Service (RaaS)</span>'
                    '<code>%s</code><div class="raas-fmts">Output as: %s</div></div>'
                    % (raas_base, fl))
        excel = ('<a class="rpt-x" href="%s?format=csv">&#128202; Export to Excel</a>'
                 % raas_base)
    else:
        ws_block = ('<div class="raas-ws off">Not enabled as a web service '
                    '(RaaS returns 403).</div>')
        excel = ""
    group_note = (' &middot; related objects are <b>clubbed together</b> within '
                  'each worker row' if has_group else "")
    body = f"""{GRID_CSS}{RAAS_TOOLBAR_CSS}<div class="card">
<div class="rpt-head">
  <div><h2 style="margin:0 0 2px">{name}</h2>
    <div class="rpt-sub">{rpt.get('report_type','')} report &middot; data source:
      {rpt.get('data_source','')}{sub_note}{crit}</div></div>
  <div class="rpt-actions">{excel}
    <a class="rpt-x" href="/task/edit-report?name={name}">&#9998; Edit Report</a>
    <span class="wd-count" id="rpttbl-count"></span></div>
</div>
{ws_block}
<p style="font-size:12px;color:#888;margin:10px 0 6px">Tip: click any column header
to Sort Ascending / Descending or Filter (like the Workday grid).{group_note}</p>
...
if __name__ == "__main__":
    ...
    print("Mock Workday tenant + UI -> http://localhost:8443/home")
