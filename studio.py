...
Open via the UI search bar ("Workday Studio") or http://localhost:8443/task/workday-studio
...
def _contains(a, b):
    if isinstance(a, dict) and "text" in a:      # message object -> its text
        a = a["text"]
...
def mvel_eval(expr, ctx):
    ...
    def _tagvalue(name):
        txt = (ctx.get("message") or {}).get("text", "") or ""
        m2 = re.search(rf"<{name}>([^<]*)</{name}>", txt)
    ...
    ns = {**ctx.get("vars", {}), **ctx, **_mvel_helpers(),
          ...
          "lp": _make_lp(ctx), "tagvalue": _tagvalue,
    ...
    tree = ast.parse(_translate_mvel(expr).strip(), mode="eval")
...
def run_assembly(assembly, isu):
        ...
        try:
            if t == "launch":
                ...
                if op == "Get_Workers":              # PagedGet pattern
                    ...
                    ctx["vars"]["workerIds"] = re.findall(
                        r'wd:type="Employee_ID">([^<]+)<',
                        "\n".join(workers))
                ...
                ext = "xml" if doc.lstrip().startswith("<") else "csv"
                fname = f"stored_{p.get('label','doc')}_" \
                        f"{time.strftime('%H%M%S')}.{ext}"
                    ...
                    for ss in sub:
                        ...
                        if st_ == "xslt":
                            ...
                            if r.status_code != 200:
                                ...
                                ctx.setdefault("error_parts", []).append(
                                    (part, f"Web service fault: {sub_fail}"))
                                pdoc = (f'<Record_Error employee-part='
                                        f'"see-request-below">{sub_fail}'
                                        f'</Record_Error>\n{pdoc}')
                            else:
                                pdoc = b2       # capture the WS response
                        elif st_ == "post_ws":
                            path = sp.get("path",
                            ...
                            if r.status_code != 200:
                                ...
                                ctx.setdefault("error_parts", []).append(
                                    (part, f"Web service fault: {sub_fail}"))
                                pdoc = (f'<Record_Error>{sub_fail}'
                                        f'</Record_Error>\n{pdoc}')
                            else:
                                pdoc = b2
                        elif st_ == "route":
                            cond = sp.get("condition", "true")
                            ctx["message"]["text"] = pdoc
                            if not bool(mvel_eval(cond, ctx)):
                                n_filtered += 1
                                ctx.setdefault("error_parts", []).append(
                                    (part, "Validation failed - routed to "
                                           "invalid path (router)"))
                                pdoc = None     # invalid path: filtered out
                                break
                        elif st_ == "log":
                ...
                wrap = assembly[agg_i].get("props", {}).get("wrapper",
                                                            "Aggregated")
                results = [re.sub(r"<\?xml[^?]*\?>\s*", "", r0)
                           for r0 in results]      # one declaration max
                doc = ('<?xml version="1.0"?>\n'
                       + (f"<{wrap}>\n" + "\n".join(results)
                          + f"\n</{wrap}>" if wrap else "\n".join(results)))
                detail = (f"Split into {len(parts_found)} <{el}> part(s); "
                          ...
                          + (f"; {n_filtered} filtered by route "
                             f"(invalid path)" if n_filtered else "")
                          + (f"; {n_fault} record-level fault(s)"
                             if n_fault else "") + ".")
                if sub_fail:
                ...
                content_kind = (p.get("content", "message") or
                                "message").strip().lower()
                if content_kind == "errors":
                    # Vendor error file: original record columns + reason,
                    # so the sender can correct and resubmit.
                    eparts = ctx.get("error_parts", [])
                    cols, rows_out = [], []
                    for part_xml, reason in eparts:
                        vals = dict(re.findall(
                            r"<(\w+)>([^<]*)</\1>", part_xml))
                        for c in vals:
                            if c not in cols:
                                cols.append(c)
                        vals["Error_Reason"] = reason
                        rows_out.append(vals)
                    hdr = cols + ["Error_Reason"]
                    body_txt = ",".join(hdr) + "\n" + "\n".join(
                        ",".join('"' + str(r0.get(c, "")).replace('"', "'")
                                 + '"' for c in hdr) for r0 in rows_out) + "\n"
                    with open(os.path.join(OUT_DIR, fname), "w") as f:
                        f.write(body_txt if eparts else
                                "No errors - all records processed "
                                "successfully.\n")
                    outputs.append(fname)
                    log.append({"step": "Out Transport: Write File (errors)",
                                "status": "OK",
                                "detail": f"Error file for vendor: "
                                          f"{len(eparts)} failed record(s) "
                                          f"-> cc_output/{fname}"})
                else:
                    with open(os.path.join(OUT_DIR, fname), "w") as f:
                        f.write(doc)
                    outputs.append(fname)
                    log.append({"step": "Out Transport: Write File",
                                "status": "OK",
    ...
    return {"log": log, "outputs": outputs,
...
function render(){
  ...
  while (i < asm.length) {
    ...
    if (s.type === 'mediation' ||
      ...
      html += conn() + `<div class="mbox ${sel===hdrIdx&&explicit?'sel':''}"
        ${explicit?`onclick="select(${hdrIdx})"`:''}>
        ...
        (steps.length ? steps.map((k,n) => {
          ...
          return `<div class="mstep ${sel===k?'sel':''}"
    ...
    html += conn() + `<div class="tbox ${i===sel?'sel':''}"
      onclick="select(${i})">
...
async function runAsm(){
  ...
  const d = await r.json();
  con.innerHTML = d.log.map(s =>
    `<div><span class="${s.status==='OK'?'ok':'fail'}">[${s.status}]</span>
     <b>${s.step}</b> - ${s.detail.replace(/</g,'&lt;')}</div>`).join('') +
    `<div style="margin-top:6px"><b>${d.result}</b>` +
    (d.outputs.length ? ' - output: ' + d.outputs.map(f =>
